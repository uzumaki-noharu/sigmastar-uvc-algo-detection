...
static ST_Rect_T vpeChnCrop = {0, 0, 0, 0};
    ...
    while(!g_bExit)
    {
        ...
        switch(select){
           case 1:
				...
				stChnCropWin.u16X = vpeChnCrop.u32X;
				stChnCropWin.u16Y = vpeChnCrop.u32Y;
				stChnCropWin.u16Width = vpeChnCrop.u16PicW;
				stChnCropWin.u16Height = vpeChnCrop.u16PicH;
...
MI_SYS_ChnPort_t stChnOutputPort1;
MI_S32 divp_s32Fd = 0;
int debug_fd = 0;
MI_U32 u32GetFramesCount = 1;
fd_set divp_read_fds;
...
MI_U32 StreamInit()
{
    ...
    memset(&stChnOutputPort1, 0, sizeof(MI_SYS_ChnPort_t));
...
#if 1
    //get fd
    stChnOutputPort1.eModId      = E_MI_MODULE_ID_DIVP;
    stChnOutputPort1.u32DevId    = 0;
    stChnOutputPort1.u32ChnId    = 1;
    stChnOutputPort1.u32PortId   = 0;
    s32Ret = MI_SYS_GetFd(&stChnOutputPort1, &divp_s32Fd);
    if (s32Ret < 0)
    {
        printf("divp MI_SYS_GetFd ch: %d, get fd. err\n", stChnOutputPort1.u32ChnId);
        return -1;
    }
#endif	
    ExecFunc(MI_SYS_SetChnOutputPortDepth(&stChnOutputPort1, 3, 4), MI_SUCCESS);


#if 1 //debug
		char szFileName[128];
		sprintf(szFileName, "divp%d.es", stChnOutputPort1.u32ChnId);
		printf("start to record %s\n", szFileName);
		debug_fd = open(szFileName, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
		if (debug_fd < 0)
		{
			printf("\n\n create %s fail.\n\n", szFileName);
		}
...
void detect_callback(std::vector<Box_t> results)
{
	...
	ST_Rect_T rgnRect1 = {0,0,DISP_WIDTH,DISP_HEIGHT};
	...
	if(u32GetFramesCount){
		FD_ZERO(&divp_read_fds);
		FD_SET(divp_s32Fd, &divp_read_fds);
		TimeoutVal.tv_sec  = 1;
		TimeoutVal.tv_usec = 0;
		s32Ret = select(divp_s32Fd + 1, &divp_read_fds, NULL, NULL, &TimeoutVal);
		if(s32Ret < 0)
		{
			printf("\n\n select failed!\n\n");
		}
		...
		{
			printf("\n\n get divp frame time out.\n\n");
		}
		else
		{
			printf("\n\n get divp frame success.\n\n"); 
			memset(&stBufInfo, 0, sizeof(MI_SYS_BufInfo_t));
			memset(&stBufHandle, 0, sizeof(MI_SYS_BUF_HANDLE));
			s32Ret = MI_SYS_ChnOutputPortGetBuf(&stChnOutputPort1, &stBufInfo, &stBufHandle);
			if(MI_SUCCESS!=s32Ret)
			{
				printf("MI_SYS_ChnOutputPortGetBuf FIAL.\n");
		
			}
			else
			{
				u32length = stBufInfo.stFrameData.u16Height * stBufInfo.stFrameData.u32Stride[0];
				//write(debug_fd, stBufInfo.stFrameData.pVirAddr[0], u32length);
...
#if 0
    vpeChnCrop.u32X = rgnRect.u32X;
    vpeChnCrop.u32Y = rgnRect.u32Y;
    vpeChnCrop.u16PicW = rgnRect.u16PicW;
    vpeChnCrop.u16PicH = rgnRect.u16PicH;
#endif					
}
...
static int FillSrcBuf(const char* FilePath,MI_DIVP_DirectBuf_t *pstDirectSrcBuf)
{
	 ...
	 if(ReadSize < SRC_WIDTH*SRC_HEIGHT*3/2)
	 {
		  ...
		  if(ReadSize < SRC_WIDTH*SRC_HEIGHT*3/2)
		  {
			  printf("read file failed, read size:%d\n",ReadSize);
			  ret = -1;
			  goto EXIT;
		  }
	}
...
static int DumpDstBuf(MI_DIVP_DirectBuf_t *pstDirectDstBuf)
{
	...
	sprintf(outputfile, "output_%dx%d_%d_%08d.yuv", pstDirectDstBuf->u32Width,
		pstDirectDstBuf->u32Height,(int)timestamp.tv_sec,(int)timestamp.tv_usec);
	...
	{
		printf("write file failed, write size:%d\n",WriteSize);
	}
	
	fflush(fp);
	sync();
	printf("save stretch dst buff to[%s]\n",outputfile);
