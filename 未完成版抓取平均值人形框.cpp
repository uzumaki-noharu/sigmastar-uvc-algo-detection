...
MI_BOOL ST_DoChangeRotate(void)
{
    ...
    for(i = 0;i < u32ArraySize;i++)
    {
        ...
        *************************************************/
        ...
        //STCHECKRESULT(MI_VPE_SetPortCrop(VpeChn, pstStreamAttr[i]->u32InputPort, &stOutCropInfo));
...
int count_haveperson = 0;
int count_noperson = 0;
int count_IOU = 0;
int count_noIOU = 0;
bool flag_find=false;
// float mean_x=0,mean_y=0,mean_width=0,mean_height=0;
// int standard_x=0,standard_y=0,standard_width=0,standard_height=0;
...
point_coord standard={0,0,0,0},cur_point={0,0,0,0};
mean total_mean={0,0,0,0};

ST_Rect_T temp = {0,0,3840,2160};


int boundary_judge(int point,int edge,int bound){
    return point+edge>bound? bound-point:edge;
}
void IOU_judge(point_coord window_1,point_coord window_2,mean& window_3){
    int IOU_x1 = window_1.x>window_2.x?window_1.x:window_2.x;
    int IOU_y1 = window_1.y>window_2.y?window_1.y:window_2.y;
    int IOU_x2 = window_1.x+window_1.width<window_2.x+window_2.width ? window_1.x+window_1.width : window_2.x+window_2.width;
    int IOU_y2 = window_1.y+window_1.height<window_2.y+window_2.height ? window_1.y+window_1.height : window_2.y+window_2.height;
    if(IOU_x1<IOU_x2 && IOU_y1<IOU_y2){
        int over_lap = (IOU_x2-IOU_x1)*(IOU_y2-IOU_y1);
        double IOU = (float)over_lap/(float)((window_1.width*window_1.height)+(window_2.width*window_2.height)-over_lap);
        std::cout<<"IOU = "<<IOU<<std::endl;
        if(IOU>=0.8){
            count_IOU++;
            window_3.x += window_1.x;
            window_3.y += window_1.y;
            window_3.width += window_1.width;
            window_3.height += window_1.height;
        }
        else {
            count_noIOU++;
        }
...
void detect_callback(std::vector<Box_t> results)
{
    ...
    MI_SYS_BUF_HANDLE   stVpeBufHandle;
    MI_SYS_BUF_HANDLE   stVencBufHandle;
    ...
    memset(&stVpeBufInfo, 0x0, sizeof(MI_SYS_BufInfo_t));
    ...
    stChnOutputPort.eModId = E_MI_MODULE_ID_VPE;
    stChnOutputPort.u32DevId = 0;
    stChnOutputPort.u32ChnId = 0;
    stChnOutputPort.u32PortId = 0;
    ...
    if(results.size()==1)
    {
        ...
        memset(&stDstBuf, 0, sizeof(MI_DIVP_DirectBuf_t));
        std::cout << "===========0============ \n"<<std::endl;
        s32Ret = MI_SYS_ChnOutputPortGetBuf(&stChnOutputPort, &stVpeBufInfo, &stVpeBufHandle);
        std::cout << "===========1============ \n"<<std::endl;
        stSrcBuf.ePixelFormat = stVpeBufInfo.stFrameData.ePixelFormat;
        stSrcBuf.u32Width = stVpeBufInfo.stFrameData.u16Width;
        stSrcBuf.u32Height = stVpeBufInfo.stFrameData.u16Height;
        memcpy(stSrcBuf.u32Stride,stVpeBufInfo.stFrameData.u32Stride,sizeof(stSrcBuf.u32Stride));
        memcpy(stSrcBuf.phyAddr,stVpeBufInfo.stFrameData.phyAddr,sizeof(stSrcBuf.phyAddr));

        // std::cout << "(void)stSrcBuf.phyAddr[0] == "<< (void *)stSrcBuf.phyAddr[0] << std::endl;
        // std::cout << "(unsigned long long)stSrcBuf.phyAddr[0] == "<< (unsigned long long)stSrcBuf.phyAddr[0] << std::endl;
        // std::cout << "stVpeBufInfo.stFrameData.phyAddr[0] == "<< stVpeBufInfo.stFrameData.phyAddr[0] << std::endl;
        // std::cout << "(void)stVpeBufInfo.stFrameData.phyAddr[0] == "<< (void *)stVpeBufInfo.stFrameData.phyAddr[0] << std::endl;
        // std::cout << "(unsigned long long)stVpeBufInfo.stFrameData.phyAddr[0] == "<< (unsigned long long)stVpeBufInfo.stFrameData.phyAddr[0] << std::endl;
        ...
        stChnInputPort.eModId = E_MI_MODULE_ID_VENC;
        stChnInputPort.u32DevId = 1;
        stChnInputPort.u32ChnId = 0;
        stChnInputPort.u32PortId = 0;

        stBufConf.eBufType = stVpeBufInfo.eBufType;
        stBufConf.stFrameCfg.eFormat = E_MI_SYS_PIXEL_FRAME_YUV_SEMIPLANAR_420;
        stBufConf.stFrameCfg.u16Width = stVpeBufInfo.stFrameData.u16Width;
        stBufConf.stFrameCfg.u16Height = stVpeBufInfo.stFrameData.u16Height;
        stBufConf.u32Flags = MI_SYS_MAP_VA;

        MI_SYS_ChnOutputPortPutBuf(stVpeBufHandle);    

        // s32Ret = MI_SYS_ChnInputPortGetBuf(&stChnInputPort, &stBufConf, &stVencBufInfo, &stVencBufHandle, 0);
        // std::cout << "===========2============ \n"<<std::endl;
        // stDstBuf.ePixelFormat = stVencBufInfo.stFrameData.ePixelFormat;
        // stDstBuf.u32Width = stVencBufInfo.stFrameData.u16Width;
        // stDstBuf.u32Height = stVencBufInfo.stFrameData.u16Height;
        // memcpy(stDstBuf.u32Stride,stVencBufInfo.stFrameData.u32Stride,sizeof(stDstBuf.u32Stride));
        // memcpy(stDstBuf.phyAddr,stVencBufInfo.stFrameData.phyAddr,sizeof(stDstBuf.phyAddr));
        // std::cout << "===========3============ \n"<<std::endl;
        // //s32Ret = MI_DIVP_StretchBuf(&stSrcBuf,&stSrcCrop,&stDstBuf);
        // std::cout << "===========4============ \n"<<std::endl;
        // //MI_SYS_ChnInputPortPutBuf(stVencBufHandle, &stVencBufInfo, FALSE);
    }
    ...
    //         count_haveperson++;
    //         count_noperson=0;
    //         cur_point.x = rgnRect.u32X<0 ? 0:rgnRect.u32X;
    //         cur_point.y = rgnRect.u32Y<0 ? 0:rgnRect.u32Y;
    //         cur_point.width = boundary_judge(cur_point.x,rgnRect.u16PicW,3840);
    //         cur_point.height = boundary_judge(cur_point.y,rgnRect.u16PicH,2160);
    //         IOU_judge(cur_point,standard,total_mean);
    //         if(count_haveperson>=10 && count_noIOU/count_haveperson>0.5){
    //             flag_find = false;
    //             count_haveperson=0;
    //             total_mean.x=0;
    //             total_mean.y=0;
    ...
    //     }
    //     else{
    //         if(count_haveperson==0){
    //         count_haveperson++;
    //         count_noperson=0;
    //         standard.x  = rgnRect.u32X<0 ? 0:rgnRect.u32X;
    //         standard.y  = rgnRect.u32Y<0 ? 0:rgnRect.u32Y;
    //         standard.width  = boundary_judge(standard.x,rgnRect.u16PicW,3840);
    //         standard.height  = boundary_judge(standard.y ,rgnRect.u16PicH,2160);
    //         total_mean.x = standard.x;
    //         total_mean.y = standard.x;
    //         total_mean.width = standard.width;
    //         total_mean.height = standard.height;
    //         }
    //         else if(count_haveperson>=30){
    //             std::cout<<"count_IOU = "<<count_IOU <<std::endl;
    //             std::cout<<"count_IOU/count_haveperson = "<<(float)count_IOU/(float)count_haveperson <<std::endl;
    //             if((float)count_IOU/(float)count_haveperson >= 0.8){
    //                 flag_find = true;
    //                 count_haveperson=0;
    //                 count_noperson=0;
    //                 count_IOU=0;
    ...
    //     }
    // }
    // std::cout<<"count_noperson= "<<count_noperson<<'\t'<<"count_haveperson= "<<count_haveperson<<std::endl;
    // std::cout<<"count_IOU = "<<count_IOU <<'\t' <<"count_noIOU = "<<count_noIOU<<'\t'<<"flag_find = "<<flag_find<<std::endl;

    // if(flag_find){
    //     temp.u32X = total_mean.x/30;
    //     temp.u32Y = total_mean.y/30;
    //     temp.u16PicW = total_mean.width/30;
    //     temp.u16PicH = total_mean.height/30;
    //     std::cout<<temp.u32X<<'\t'<<temp.u32Y<<'\t'<<temp.u16PicW<<'\t'<<temp.u16PicH<<std::endl;
    //     ST_OSD_DrawRect(rgnHandle, temp, RECT_BORDER_WIDTH, 2);
    // }
    // std::cout<<std::endl;
...
static MI_S32 UVC_StartCapture(void *uvc,Stream_Params_t format)
{
    ...
    ************************************************/
    MI_U32 VencChn = pstStreamAttr[dev->dev_index].vencChn;
    std::cout<<'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!VencChn ='<<VencChn<<std::endl;
