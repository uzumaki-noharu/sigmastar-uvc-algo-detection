...
int num_valid = 0;
int num_unvalid = 0;
MI_U32 TMP_X = 0, TMP_Y = 0;
clock_t start_time = clock();

MI_U32 BaseTop = 16;
MI_U32 BaseLeft = 16;
MI_U32 dis = DISP_HEIGHT;
MI_U32 current_x = 0, current_y = 0;
MI_U32 move_step_x = 32,move_step_y = 16;
MI_U32 edg_x_min = 350,edg_x_max = 450;
MI_U32 edg_y_min = 150, edg_y_max = 250;
MI_U32 show_w_max = 3520, show_h_max = 1760;
MI_U32 show_w = 3520, show_h = 1760;
MI_U32 width_max = 3840;
MI_U32 height_max = 2160;
clock_t fps_time = clock();
int flage_trace = 0;
double fps = 0;
char str[7];

MI_U32 distance(MI_U32 point1_x,MI_U32 point1_y,MI_U32 point2_x,MI_U32 point2_y)
{
    MI_U32 x;
    MI_U32 y;
    MI_U32 result;
    x = (point1_x > point2_x)?(point1_x - point2_x) :(point2_x - point1_x) ;
    y = (point1_y > point2_y)?(point1_y - point2_y) :(point2_y - point1_y);
    result = sqrt(x*x + y*y);
    return result;
}
...
void detect_callback(std::vector<Box_t> results)
{
    ...
    MI_U32 BaseH = 2;
    MI_U32 compare_x_min = 0 , compare_x_max=0;
    MI_U32 compare_y_min = 0 , compare_y_max=0;
    ...
    if (results.size() == 1)
    {
        num_valid += 1;
        num_unvalid = 0;
        if (clock() - start_time > 2*CLOCKS_PER_SEC)
        {
            dis = distance(rgnRect.u32X,rgnRect.u32Y,TMP_X,TMP_Y);
            TMP_X = rgnRect.u32X;
            TMP_Y = rgnRect.u32Y;
            start_time = clock();
        }
    ...
    if(num_valid >40 && dis <200)
    {
        // edg_x_min = MI_U32(show_w/6);
        // edg_x_max = MI_U32(show_w/4);
        // std::cout<< "edg_x_min:"<<edg_x_min<<"  edg_x_max"<<edg_x_max<<std::endl;
        // edg_y_min = MI_U32(show_h/6); 
        // edg_y_max = MI_U32(show_h/4);
        std::cout<< "edg_y_min:"<<edg_y_min<<"  edg_y_maxx"<<edg_y_max<<std::endl;
        compare_x_min = (rgnRect.u32X > edg_x_min) ? (rgnRect.u32X - edg_x_min) : 0;
        compare_x_max = (rgnRect.u32X > edg_x_max) ? (rgnRect.u32X - edg_x_max) : 0;
        std::cout<< "compare_x_min:"<<compare_x_min<<"  compare_x_max"<<compare_x_max<<std::endl;
        compare_y_min = (rgnRect.u32Y > edg_y_min) ? (rgnRect.u32Y - edg_y_min) : 0;
        compare_y_max = (rgnRect.u32Y > edg_y_max) ? (rgnRect.u32Y - edg_y_max) : 0;
        std::cout<< "compare_y_min:"<<compare_y_min<<"  compare_y_max"<<compare_y_max<<std::endl;
        ...
        if (show_w > 1920) 
        {
            ...
            show_w = show_w -  32;
            show_h = show_h -  16;
            std::cout<<"dadadadada-----"<< "show_w:"<<show_w<<"  show_h"<<show_h<<std::endl;
           
        }
        
        
        if (current_x > compare_x_min)
        {
            printf("@@@@@@@@@@@@@@@Left\n");
            current_x = (current_x > move_step_x)?(current_x - move_step_x):0;
        }
        if (current_x < compare_x_max)
        {
            printf("@@@@@@@@@@@@@@Right\n");
            current_x = current_x + move_step_x;
        }


        if (current_y > compare_y_min)
        {
            printf("@@@@@@@@@@@@@@Up\n");
            current_y = (current_y > move_step_y)?(current_y - move_step_y):0;
        }
        if (current_y < compare_y_max)
        {
            printf("@@@@@@@@@@@@@@Down\n");
            current_y = current_y + move_step_y;
        }
    ...
    // std::cout<< "show_w:"<<show_w<<"  show_h:"<<show_h<<std::endl;
    // std::cout<< "width_max:"<<width_max<<"  height_max:"<<height_max<<std::endl;
    // current_x = ((current_x + show_w) > width_max) ? (width_max-show_w): current_x;
    // current_y = ((current_y + show_h) > height_max) ? (height_max-show_h): current_y;
    
    // std::cout<< "currnt_x:"<<current_x<<"  rgnRect.u32X:"<<rgnRect.u32X<<std::endl;
    // std::cout<< "currnt_y:"<<current_y<<"  rgnRect.u32Y:"<<rgnRect.u32Y<<'\n'<<std::endl;

    // if ((rgnRect.u32X - current_x <= edg_x_max) && (rgnRect.u32Y - current_y <= edg_y_max))
    // {
    //     flage_trace = flage_trace + 1;
    // }
    // else
    // {
    //     flage_trace = 0;
    ...
    // if (flage_trace > 30)
    // {
    //     int tmp = (flage_trace-30 > 25)?25:(flage_trace-30);
    //     show_w = show_w - tmp * 64;
    //     show_h = show_h - tmp * 32;
    //     if (rgnRect.u32X - current_x > edg_x_min)
    //     {
    //         current_x = current_x + tmp * 16;
    //     }
    //     if (rgnRect.u32Y - current_y > edg_y_min)
    //     {
    //         current_y = current_y + tmp * 16;
    //     }
    // }

    if (num_unvalid>30)
    {
        if (show_w < show_w_max)
        {
            show_w = show_w + 32;
            show_h = show_h + 16;
            std::cout<<"xiaoxiaoxiao-----"<< "show_w:"<<show_w<<"  show_h"<<show_h<<std::endl;
            current_x = (current_x > move_step_x)?(current_x - move_step_x): 0;
            current_y = (current_y > move_step_y)?(current_y - move_step_y) : 0;
        }
        
        
        std::cout<< "xiaoxiao-----currnt_x:"<<current_x<<"  rgnRect.u32X:"<<rgnRect.u32X<<std::endl;
        std::cout<< "xiaoxiao-----currnt_y:"<<current_y<<"  rgnRect.u32Y:"<<rgnRect.u32Y<<'\n'<<std::endl;
        
    }

    show_w = (show_w_max < show_w) ? show_w_max:show_w;
    show_h = (show_h_max < show_h) ? show_h_max:show_h;
    show_w = (show_w < 1920) ? 1920:show_w;
    show_h = (show_h < 960) ? 960:show_h;
    std::cout<<"lastlast-----"<< "show_w:"<<show_w<<"  show_h:"<<show_h<<std::endl;
    
    vencCrop.stRect.u32Left=current_x - (current_x % BaseLeft);
    vencCrop.stRect.u32Top=current_y - (current_y % BaseTop);
    current_x = (current_x < 0) ? 0 : current_x;
    current_y = (current_y < 0) ? 0 : current_y;
    current_x = ((current_x + show_w) >= width_max) ? (width_max-show_w-32): current_x;
    current_y = ((current_y + show_h) >= height_max) ? (height_max-show_h-16): current_y;

    vencCrop.stRect.u32Width=show_w;
    vencCrop.stRect.u32Height=show_h;
    std::cout<< "last currnt_x:"<<current_x<<"  rgnRect.u32X:"<<rgnRect.u32X<<std::endl;
    std::cout<< "last currnt_y:"<<current_y<<"  rgnRect.u32Y:"<<rgnRect.u32Y<<'\n'<<std::endl;
    ...
    ST_Point_T point;
    fps = (clock() - fps_time);
    std::cout<< "second:"<<fps<<std::endl;
    fps = 1/fps;
    fps = fps * CLOCKS_PER_SEC;
    sprintf(str, "%.2f", fps);
    std::cout<< "fps:"<<fps<<"  str:"<<str<<'\n'<<std::endl;
    point.u32X = 64;
    point.u32Y = 64;
    fps_time = clock();
    ST_OSD_DrawText(rgnHandle,point,str,2,DMF_Font_Size_32x32);
